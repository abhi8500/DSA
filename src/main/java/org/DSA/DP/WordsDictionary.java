...
public class WordsDictionary {
    ...
    public static void main(String[] args){
       ...
       System.out.println( WordsDictionary.numWaysTabu(words,"ac"));

    }

    // DP Iterative Approch
    private static int numWaysTabu(String[] words, String target) {
        ...
        for (int currWord = 1; currWord <= wordLength; ++currWord) {
            for (int currTarget = 1; currTarget <= targetLength; ++currTarget) {
                ...
                dp[currWord][currTarget] += (charFrequency[currWord - 1][curPos] *
                        dp[currWord - 1][currTarget - 1]) %
                        MOD;

                System.out.println("currWord " + currWord + " currTarget " + currTarget
                        + " " + dp[currWord][currTarget]+" Map "+charFrequency[currWord - 1][curPos]);
